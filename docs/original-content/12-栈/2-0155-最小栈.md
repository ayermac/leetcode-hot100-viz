...
大家好，我是小编。今天我们来深入探讨一道非常精彩的数据结构设计题 - LeetCode 155「最小栈」。这道题不仅考察了栈的基本操作，更蕴含着优化思维的艺术。
...
func (s *MinStack) Push(val int) {
    s.mainStack = append(s.mainStack, val)
...
func (s *MinStack) Push(val int) {
    if len(s.stack) == 0 {
        ...
        s.stack = append(s.stack, int64(val)-s.min)
        if int64(val) < s.min {
            s.min = int64(val)
        }
    }
