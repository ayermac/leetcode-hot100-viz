...
func pathSum(root *TreeNode, targetSum int) int {
    ...
    return countPath(root, int64(targetSum)) +
        pathSum(root.Left, targetSum) +
        pathSum(root.Right, targetSum)
}

func countPath(node *TreeNode, targetSum int64) int {
    if node == nil {
        return 0
    }
    ...
    // 递归计算左右子树
    count += countPath(node.Left, targetSum-int64(node.Val))
    count += countPath(node.Right, targetSum-int64(node.Val))
    return count
}
```
**复杂度分析**  
...
func pathSum(root *TreeNode, targetSum int) int {
    ...
    prefixSumMap[0] = 1 // 初始化
    return dfs(root, 0, int64(targetSum), prefixSumMap)
}

func dfs(node *TreeNode, currentSum, targetSum int64, prefixSumMap map[int64]int) int {
    if node == nil {
        return 0
    }

    currentSum += int64(node.Val) // 更新当前路径和
    count := prefixSumMap[currentSum-targetSum] // 查找满足条件的路径
