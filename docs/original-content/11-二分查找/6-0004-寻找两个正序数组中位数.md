...
大家好，我是小编。今天我们要挑战一道 LeetCode 难度系数最高的题目之一 - LeetCode 4「寻找两个正序数组的中位数」。这道题不仅考察我们对二分查找的理解，更要求我们具备深入的数学思维。让我们一起攻克这个难题！
...
**示例**：
```go
输入：nums1 = []int{1,3}, nums2 = []int{2}
输出：2.0
解释：合并数组 = []int{1,2,3}，中位数是 2.0

输入：nums1 = []int{1,2}, nums2 = []int{3,4}
输出：2.5
解释：合并数组 = []int{1,2,3,4}，中位数是 (2 + 3) / 2 = 2.5
```
...
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
    ...
    for left <= right {
        ...
        // 获取分割线左右的元素
        maxLeft1 := float64(math.MinInt64)
        if i > 0 {
            maxLeft1 = float64(nums1[i - 1])
        }
        
        minRight1 := float64(math.MaxInt64)
        if i < m {
            minRight1 = float64(nums1[i])
        }
        
        maxLeft2 := float64(math.MinInt64)
        if j > 0 {
            maxLeft2 = float64(nums2[j - 1])
        }
        
        minRight2 := float64(math.MaxInt64)
        if j < n {
