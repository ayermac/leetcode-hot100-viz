...
大家好，我是小编。今天要和大家分享一道特别有趣的题目 - LeetCode 33「搜索旋转排序数组」。这道题巧妙地将二分查找与旋转数组结合，是一道考察思维灵活性的经典题目。
...
**示例**：
```go
输入：nums = []int{4,5,6,7,0,1,2}, target = 0
输出：4  // 0在位置4

输入：nums = []int{4,5,6,7,0,1,2}, target = 3
输出：-1  // 3不存在于数组中
```
